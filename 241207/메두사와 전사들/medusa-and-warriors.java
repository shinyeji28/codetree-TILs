...
public class Main {
    ...
    static List<Integer> medusaRoute = new ArrayList<>();   // 메두사 최단거리
    static int[][] rocks;
    ...
    public static void solution(int sx, int sy,int ex, int ey){
        ...
        while(true){
            ...
            int[] cur = movingMedusa(x,y, idx);  // 공원도착 종료
            idx++;
            x = cur[0];
            y = cur[1];
            if(x == ex && y == ey){
    ...
    public static int[] movingMedusa(int x, int y, int idx){
        int medusaIdx = medusaRoute.get(idx);
        int nx = medusaIdx / n;
        int ny = medusaIdx % n;
    ...
    public static void reversePath(int[] path, int sx,int sy,int ex,int ey){
        int x = ex;
        int y = ey;
        while(!(x == sx && y == sy)){
            int np = path[x * n + y];
            int nx = np/n;
            int ny = np%n;
            medusaRoute.add(nx * n + ny);
            x = nx;
            y = ny;
        }
        Collections.reverse(medusaRoute);
        medusaRoute.add(ex * n + ey);       
